...
- **학급 공통 피드백 & 피드백 알림 (v34~v38, v47(2026-09-09)에 오개념 섹션 추가, v49(2026-09-10)에
  범위를 "전체 반"→"내 담당 반+개별 반"으로 정정)**: 교사 대시보드 "학생 기록" 탭에서 학년+반+활동(하나)을
  모두 골랐을 때만 `#classFbBox`가 나타나(`dashRenderClassFeedback()`, `index.html:3467` 부근) 그
  범위에 해당하는 학생 글을 모아 AI로 "교사용 리포트"·"학생 공지용 문구"·"대표적인 오개념"을 생성한다
  (`generateClassFeedback` action, 조회는 `mode=classFeedback`). 학생 공지용 문구는 자동으로 학생 포털
  활동 완료 카드에도 표시되고, **오개념은 교사 전용**이라 학생 화면엔 절대 안 내려간다. 반은 숫자 하나
  또는 **"내 담당 반"**(`selBan==='mine'`) — `dashClassFeedbackBanParam_()`이 그 시점의 `dashMyBans_()`
  결과를 콤마 문자열("5,6,7,8")로 풀어서 서버에 보낸다(공지 대상 "담당 학급반", v30과 같은 관례). **'전체
  반'(`ban===''`)은 v47에서 잠깐 지원했다가 v49에서 뺐다** — 효니가 "학급 단위"가 아닌 범위는 원하지
  않아서, "내 담당 반"으로 바꿔 되돌림. **`generateClassFeedback`/`mode=classFeedback`은 2026-09-09~10에
  `history26_backend`에 반영·배포 완료.**
...
- **복습용 클래스카드 (v45, 2026-09-09)**: `#portfolioCard`(패들렛 포트폴리오)와 완전히 같은 구조로
  `#classcardCard`를 하나 더 추가했다 — 공지로 올리면 피드에 묻혀 학생이 다시 못 찾으니, 패들렛처럼
  학기 내내 포털에 고정 노출되는 카드가 필요해서다. `cur.classcard`(`{title, desc, url}` 또는 반별로
  다르면 `urlByBan`)가 있고 그 반의 URL이 비어있지 않을 때만 렌더링되며(`renderPortal()`,
  `index.html:1219` 부근), 없으면 패들렛 카드처럼 조용히 숨는다. 값 자체(제목·설명·초대코드 안내 문구·
  링크)는 코드가 아니라 교사 대시보드 "커리큘럼 관리 > 학년정보" 탭의 새 입력칸(`gi-cc-title`/
  `gi-cc-desc`/`gi-cc-url`, `dashSaveGradeInfo()`)에서 학년별로 직접 입력·저장한다 — 포트폴리오와
...
- **체크인 문항 "지난 체크인" 표시 (v53, 2026-09-14 — v52 방식에 대한 효니 정정)**: v52의
  ...
  - **서버 쪽(`체크인문항` 시트에 `지난여부` 컬럼 추가, `toggleCheckinLessonPast` action,
    ...
    반영되지 않는다 — 배포 후 정상 동작.
- **성적조회 (v54, 2026-09-15 신규 — 핸드오프 "성적조회 history26 포털 통합 v2", 옵션 A 채택)**:
  별도 GAS 스크립트로 존재하던 "엑셀 성적표 → 학생 개인 조회" 기능을 포털에 통합했다. **포털
  로그인(`login()`)은 전혀 안 건드림** — 학생 포털 "그 밖의 기록" 영역에 `#gradeCard`(포트폴리오·
  클래스카드와 같은 `.portfolio-card` 구조 재사용, `index.html:189` 부근)를 새로 추가해서, 그
  안에서만 학번+이름(이미 `SESSION`에 있음)에 **비밀번호까지 3중 매칭**을 새로 요구한다(`mode=grade`,
  `openGradeGate()`/`submitGradeLookup()`). 비밀번호는 자동로그인 캐싱 없이 조회할 때마다 새로
  입력받는다. 매칭 실패 사유(학번/이름/비밀번호 중 뭐가 틀렸는지)는 서버가 이미 뭉뚱그려 반환하므로
  프론트는 그 메시지를 그대로 보여줄 뿐 원인을 추측하는 로직을 두지 않는다(무차별 대입 단서 차단).
  교사 대시보드에는 새 탭 "🧮 성적관리"(`panelGrades`)가 생겨 성적 엑셀 업로드(`dashUploadGradeExcel()`
  — 파일을 base64로 읽어 `uploadGradeExcel` action으로 전송)와 열람 상태 ON/OFF 토글
  (`dashSetGradeDisplayStatus()`)을 제공한다. **학생별 비밀번호 관리(등록·수정·리셋) UI는 의도적으로
  안 만들었다** — 핸드오프 확정 사항대로 효니가 "계정" 시트를 구글시트에서 직접 편집한다.
  ⚠️ 매칭키를 원본 스크립트의 "반+번호"에서 history26 5자리 학번으로 통일했으므로, 업로드하는 성적
  엑셀의 학번 열도 5자리 형식이어야 조회된다 — 대시보드 안내 문구로만 알려주고 코드로 강제하지는
  않는다. **서버 쪽(`계정` 시트, `mode=grade`/`gradeStatus`, `action=uploadGradeExcel`/
  `setGradeDisplayStatus`, `DEBUG_MODE` 스크립트 속성)은 이 저장소에 없다** — `history26_backend`,
  아직 배포 대기(Drive API 고급 서비스를 Apps Script 편집기에서 수동으로 켜야 엑셀 업로드가 동작함,
  자세한 내용은 그 레포 CLAUDE.md 참고). 비밀번호는 평문 저장(해시 없음) — 위협 모델이 "동급생이
  서로 훔쳐보는 것" 수준이라 이 정도로 충분하다고 판단한 핸드오프의 명시적 결정.
...
- 코드 곳곳의 주석에 `v12`, `v17`, `v22`, `v27`, `v29`처럼 버전 표기가 붙어 있고, 그 버전에서 왜 그렇게
  ...
  (커리큘럼의 지난 활동 표시와 동일 방식), v54 = 성적조회 기능 통합(옵션 A — 포털 로그인은
  무변경, 학생 포털에 비밀번호 3중 매칭 조회 카드 추가 + 교사 대시보드 "성적관리" 탭 신설)
  (아래 각각 참고)
  — 확인 시점 기준
  `index.html`/`style.css`에서 가장 높은
  버전 표기는 v54). 여러 기능이 같은 버전
