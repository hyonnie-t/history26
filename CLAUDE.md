...
- **학급 공통 피드백 & 피드백 알림 (v34~v38, 2026-09-09에 오개념 섹션·전체 반 범위 추가)**: 교사 대시보드
  ...
  아님) 또는 **v45부터 '전체 반'(`ban===''`)도 허용** — 이땐 반 구분 없이 그 학년+활동 전체 학생 글을
...
- 코드 곳곳의 주석에 `v12`, `v17`, `v22`, `v27`, `v29`처럼 버전 표기가 붙어 있고, 그 버전에서 왜 그렇게
  ...
  높이 계산을 CSS의 vh 고정값에서 JS 실측 기반으로 교체, v45 = 학생 기록 탭 활동 선택 유지
  버그 수정 + 학급 공통 피드백에 오개념 섹션·전체 반 범위 추가(아래 참고) — 확인 시점 기준
  `index.html`/`style.css`에서 가장 높은 버전 표기는 v45). 여러 기능이 같은 버전
