...
- **학생 포털 구조 재배치 (v42, 2026-09-08 — 효니 진단 요청에 따른 리디자인)**: `#portalView`가
  ...
  3. `.layout`이 `max-width:1024px`에서 2단→1단으로 붕괴할 때(아이패드 세로모드 대부분·구형 아이패드
     가로모드 포함) 소스 순서상 먼저 오는 사이드카드가 화면 맨 위를 차지하던 것을, `.side-col{order:2}`/
     `main{order:1}`로 화면 노출 순서만 뒤집어 해결했다(`style.css:299` 부근) — 마크업은 그대로라
     1025px 이상(데스크톱)에서는 기존과 동일하게 좌측 사이드카드가 그대로 보인다.
...
- 코드 곳곳의 주석에 `v12`, `v17`, `v22`, `v27`, `v29`처럼 버전 표기가 붙어 있고, 그 버전에서 왜 그렇게
  ...
  (아래 참고) — 확인 시점 기준 `index.html`/`style.css`에서 가장 높은 버전 표기는 v42). 여러 기능이 같은 버전
