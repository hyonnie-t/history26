...
- **개별 수업 웹앱은 이 저장소 밖에서 각자 따로 만들어진다.** `crusades`(십자군, 화면 하나짜리
  ...
  없이 fetch 한 번 실패로 끝나 활동 중 시트 기록이 안 남을 뻔함). ④ 학생이 읽는 안내 텍스트에
  걸려 넘어질 만한 한자어 개념어가 있으면 어휘 힌트도 넣는다 — **괄호 병기(예: "시인(인정)")는
  가독성 저하로 반려됐고(2026-09-10, 효니 피드백)**, 단어를 밑줄 표시해 누르면 화면 하단에
  토스트로 뜻이 뜨는 클릭식으로 통일한다(`reformation_debate`에서 최초 구현·검증됨). 자세한
  내용은 스킬 파일 참고.
