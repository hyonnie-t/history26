...
- **학급 공통 피드백 & 피드백 알림 (v34~v38, v47(2026-09-09)에 오개념 섹션·전체 반 범위 추가)**: 교사 대시보드
  "학생 기록" 탭에서 학년+반+활동(하나)을 모두 골랐을 때만 `#classFbBox`가 나타나(`dashRenderClassFeedback()`,
  `index.html:3396` 부근) 그 범위에 해당하는 학생 글을 모아 AI로 "교사용 리포트"·"학생 공지용 문구"·
  "대표적인 오개념"을 생성한다(`generateClassFeedback` action, 조회는 `mode=classFeedback`). 학생 공지용
  문구는 자동으로 학생 포털 활동 완료 카드에도 표시되고, **오개념은 교사 전용**이라 학생 화면엔 절대 안
  내려간다. 반은 숫자 하나("담당 학급반mine" 묶음은 여전히 제외 — 교사마다 다르고 안정적인 저장 키가
  아님) 또는 **v47부터 '전체 반'(`ban===''`)도 허용** — 이땐 반 구분 없이 그 학년+활동 전체 학생 글을
  합쳐서 분석한다(교사가 "학급 단위"가 아니라 "활동 전체에서의 대표 경향"을 보고 싶을 때 씀).
  **`generateClassFeedback`/`mode=classFeedback` 코드는 `history26_backend`(`code.gs`)에 이미 있지만
  아직 실제 서비스에 배포 전**이며, 배포 전까지 이 박스는 "아직 없음" 상태로만 보이거나 생성 버튼이 실패
  토스트를 낸다(배포 전 옛 프롬프트 결과가 오면 오개념 필드는 빈 문자열이라 그 섹션만 자동으로 숨음).
